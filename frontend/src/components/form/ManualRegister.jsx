...
export default function ManualRegister({ isOpen, onClose, alumnoAEditar }) {
  ...
  const [formData, setFormData] = useState({
    ...
    nivel_id: 1, titulation_status_id: '', 
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData({
        ...
        nivel_id: 1, titulation_status_id: '',
        career_id: '', origin_school_id: '', promedio_procedencia: '',
        calle: '', numero_domicilio: '', colonia: '', codigo_postal: '', 
        municipio: '', estado: 'Campeche', status: 'activo',
        ...
        client.get(`/alumnos/detalle/${alumnoAEditar.matricula}`).then(res => {
           ...
           let email_inst_limpio = student.email_institucional || '';
            ...
            const dataCargada = {
                ...
                nivel_id: student.nivel_id || 1, 
                titulation_status_id: student.titulation_status_id || '', 
                career_id: student.career_id, origin_school_id: student.origin_school_id, promedio_procedencia: promedioValor,
            ...
            let mensajeErrorCurp = ''; let exitoCurp = false;
    ...
    if (name === 'curp') {
      ...
      if (finalValue.length === 18) {
        if (!curpRegex.test(finalValue)) {
          ...
          const letrasEsperadas = formData.nombre && formData.apellido_paterno ? calcularLetrasCURP(formData.nombre, formData.apellido_paterno, formData.apellido_materno) : null;
          if (letrasEsperadas) {
  ...
  const handleNivelChange = (selectedOption) => {
    setFormData(prev => ({
      ...prev,
      nivel_id: selectedOption.value,
      origin_school_id: '', // Limpiamos la escuela porque cambia el catálogo
      career_id: '', // Limpiamos carrera
      titulation_status_id: '' // Limpiamos estatus de titulación
    }));
  };
  ...
  const handleFileChange = (e) => {
    ...
    if (name === 'foto') {
      ...
      if (!validTypes.includes(file.type)) { Swal.fire('Formato Inválido', 'JPG o PNG únicamente.', 'error'); e.target.value = ''; return; }
      if (file.size > 2 * 1024 * 1024) { Swal.fire('Muy pesado', 'Máximo 2MB.', 'error'); e.target.value = ''; return; }

      const img = new Image(); const objectUrl = URL.createObjectURL(file);
      img.onload = () => {
        URL.revokeObjectURL(objectUrl); 
        if (img.width < 250 || img.height < 250) { Swal.fire('Dimensiones', 'Mínimo 250x250 px.', 'error'); e.target.value = ''; return; }
        setFiles(prev => ({ ...prev, [name]: file }));
      };
      img.onerror = () => { URL.revokeObjectURL(objectUrl); Swal.fire('Error', 'Imagen corrupta.', 'error'); e.target.value = ''; };
      img.src = objectUrl;
      return;
    }
  ...
  const handleCheckCurp = async () => {
    if (formData.curp.length === 18 && (!alumnoAEditar || formData.curp !== alumnoAEditar.curp)) {
      ...
      if (!curpRegex.test(formData.curp)) {
        setErroresEnVivo(prev => ({ ...prev, curp: '❌ El formato de la CURP es incorrecto.' })); setValidacionExitosa(prev => ({ ...prev, curp: false })); return;
      }
      try {
        ...
        if (res.data.exists) {
          ...
          setErroresEnVivo(prev => ({ ...prev, curp: '' })); setValidacionExitosa(prev => ({ ...prev, curp: true })); 
        } else {
          setErroresEnVivo(prev => ({ ...prev, curp: '' })); setValidacionExitosa(prev => ({ ...prev, curp: true })); 
        }
      } catch (error) {}
  ...
  const handleCheckEmail = async () => {
    if (formData.email_personal.length > 5 && (!alumnoAEditar || formData.email_personal !== alumnoAEditar.email_personal)) {
      const emailRegex = /^[a-zA-Z0-9._%+-]+@[a-zA-Z0-9.-]+\.(com|mx|org|net|edu|gob)$/i;
      if (!emailRegex.test(formData.email_personal)) return;
      try {
        const res = await client.get(`/alumnos/check-email?email=${formData.email_personal.toLowerCase()}`);
        if (res.data.exists) {
          setErroresEnVivo(prev => ({ ...prev, email: '❌ Este correo ya está en uso.' })); setValidacionExitosa(prev => ({ ...prev, email: false }));
        } else {
          setErroresEnVivo(prev => ({ ...prev, email: '' })); setValidacionExitosa(prev => ({ ...prev, email: true })); 
        }
      } catch (error) {}
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.nombre && formData.apellido_paterno && formData.curp.length === 18) {
      ...
      if (letrasEsperadas) {
        if (formData.curp.substring(0, 4) !== letrasEsperadas.prefijo) return Swal.fire('Inconsistencia', `Las primeras 4 letras de la CURP deberían ser ${letrasEsperadas.prefijo}.`, 'error');
        if (formData.curp.substring(13, 16) !== letrasEsperadas.consonantes) return Swal.fire('Inconsistencia', `Las consonantes internas deberían ser ${letrasEsperadas.consonantes}.`, 'error');
      }
    }

    if (erroresEnVivo.curp || erroresEnVivo.email || erroresEnVivo.promedio) return Swal.fire('Formulario Incompleto', 'Corrige los errores antes de continuar.', 'warning');
    ...
    if (formData.nivel_id === 2 && !formData.titulation_status_id) {
    ...
    if (!files.foto && !tieneFotoPrevia && !esEstatusExento) return Swal.fire('Falta la Fotografía', 'La fotografía es obligatoria.', 'warning');
...
//  Filtros dinámicos y opciones para los menús

  const isMaestria = formData.nivel_id === 2;
  ...
  const schoolOptions = schools
    .filter(s => {
      const tipoLimpio = String(s.tipo || '').trim().toUpperCase();
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <form onSubmit={handleSubmit} className="space-y-8 bg-slate-50 p-2 md:p-6 rounded-lg">
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <input name="nombre" value={formData.nombre} maxLength={50} onChange={handleChange} className="w-full border border-gray-300 rounded-md p-2.5 text-sm focus:border-[#1e3a8a] outline-none" required />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-600 mb-1">Apellido Paterno <span className="text-red-500">*</span></label>
              <input name="apellido_paterno" value={formData.apellido_paterno} maxLength={50} onChange={handleChange} className="w-full border border-gray-300 rounded-md p-2.5 text-sm focus:border-[#1e3a8a] outline-none" required />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-600 mb-1">Apellido Materno <span className="text-red-500">*</span></label>
              <input name="apellido_materno" value={formData.apellido_materno} maxLength={50} onChange={handleChange} className="w-full border border-gray-300 rounded-md p-2.5 text-sm focus:border-[#1e3a8a] outline-none" required />
            </div>
            <div>
              ...
              <div className="flex justify-between items-start mt-1">
                ...
                <span className="text-xs font-mono text-gray-400">{formData.curp.length}/18</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="md:col-span-2">
              ...
              <Select 
                ...
                value={levelOptions.find(opt => opt.value === formData.nivel_id) || levelOptions[0]} 
                styles={customSelectStyles} 
              />
            </div>

            <div className="md:col-span-2">
              <label className="block text-xs font-bold text-gray-600 mb-1">Programa Académico ({isMaestria ? 'Maestría' : 'Licenciatura'}) <span className="text-red-500">*</span></label>
            ...
            {/*  Etiqueta dinámica (Prepa vs Universidad) */}
            <div className={isMaestria ? "col-span-1" : "md:col-span-2"}>
              <label className="block text-xs font-bold text-gray-600 mb-1">{isMaestria ? 'Universidad' : 'Preparatoria'} de Procedencia <span className="text-red-500">*</span></label>
            ...
            {/*  Estatus de Titulación (Solo Maestría) */}
            {isMaestria && (
              <div className="col-span-1 animate-in fade-in zoom-in-95 duration-300">
        ...
        <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-100">
          <h4 className="flex items-center text-[#1e3a8a] font-bold mb-6 text-base">
            <div className="flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 text-blue-600 mr-3">📍</div> Dirección y Contacto
          </h4>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div>
            ...
            <div>
              ...
              <input type="email" name="email_personal" value={formData.email_personal} onChange={handleChange} onBlur={handleCheckEmail} onKeyDown={(e) => e.key === ' ' && e.preventDefault()}
                className={`w-full border rounded-md p-2.5 text-sm lowercase outline-none transition-all ${erroresEnVivo.email ? 'border-red-500 bg-red-50' : 'border-gray-300 focus:border-[#1e3a8a]'}`} required />
            ...
            <div className="md:col-span-2 mt-4 border-t border-gray-100 pt-5 grid grid-cols-1 md:grid-cols-2 gap-5">
              <div className="md:col-span-2"><label className="block text-xs font-bold text-gray-600 mb-1">Calle *</label><input name="calle" value={formData.calle} onChange={handleChange} className="w-full border border-gray-300 rounded-md p-2.5 text-sm outline-none" required /></div>
              <div><label className="block text-xs font-bold text-gray-600 mb-1">Número Ext. *</label><input name="numero_domicilio" value={formData.numero_domicilio} onChange={handleChange} className="w-full border border-gray-300 rounded-md p-2.5 text-sm outline-none" required /></div>
              <div>
                <label className="block text-xs font-bold text-gray-600 mb-1">C.P. (Estado) *</label>
                <div className="flex"><span className="inline-flex items-center px-3 rounded-l-md border border-r-0 border-gray-300 bg-gray-50 text-gray-500 font-bold text-sm">24</span><input name="codigo_postal" value={formData.codigo_postal} maxLength={3} onChange={handleChange} className="w-full border border-gray-300 rounded-r-md p-2.5 text-sm outline-none font-bold" required /></div>
              </div>
              <div><label className="block text-xs font-bold text-gray-600 mb-1">Colonia *</label><Select name="colonia" options={coloniaOptions} onChange={handleSelectChange} value={coloniaOptions.find(opt => opt.value === formData.colonia) || null} placeholder={coloniasAPI.length > 0 ? "Seleccione..." : "Escriba C.P."} styles={customSelectStyles} isDisabled={coloniasAPI.length === 0} /></div>
              <div><label className="block text-xs font-bold text-gray-600 mb-1">Municipio *</label><input name="municipio" value={formData.municipio} onChange={handleChange} readOnly={coloniasAPI.length > 0 && formData.municipio !== ''} className="w-full border border-gray-300 rounded-md p-2.5 text-sm outline-none bg-gray-50" required /></div>
            </div>
          </div>
        </div>
        ...
        {/* 4. MULTIMEDIA */}
        <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-100">
          <h4 className="flex items-center text-[#1e3a8a] font-bold mb-4 text-base"><div className="flex items-center justify-center w-8 h-8 rounded-lg bg-blue-50 text-blue-600 mr-3">📁</div> Expediente Digital</h4>
          <div className="grid grid-cols-2 gap-5">
            <div className="relative group">
              <input type="file" name="foto" id="f-foto" accept="image/jpeg, image/png" onChange={handleFileChange} className="hidden" />
              <label htmlFor="f-foto" className={`flex flex-col items-center justify-center h-24 border-2 border-dashed rounded-lg cursor-pointer transition-all ${files.foto ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-300 bg-gray-50 hover:border-blue-400'}`}>
                <span className="text-xs font-bold uppercase tracking-wider text-center px-2">{files.foto ? `✅ ${files.foto.name}` : alumnoAEditar ? (formData.foto_id ? `📷 Fotografía guardada` : "Agregar Fotografía (Opcional)") : "Subir Fotografía *"}</span>
              </label>
            </div>
            <div className="relative group">
              <input type="file" name="certificado" id="f-cert" accept=".pdf" onChange={handleFileChange} className="hidden" />
              <label htmlFor="f-cert" className={`flex flex-col items-center justify-center h-24 border-2 border-dashed rounded-lg cursor-pointer transition-all ${files.certificado ? 'border-green-500 bg-green-50 text-green-700' : 'border-gray-300 bg-gray-50 hover:border-blue-400'}`}>
                <span className="text-xs font-bold uppercase tracking-wider text-center px-2">{files.certificado ? `✅ ${files.certificado.name}` : alumnoAEditar ? (formData.certificado_id ? `📄 Certificado guardado` : "Agregar Certificado") : "Subir Certificado"}</span>
              </label>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center pt-4">
          ...
          <div className="flex space-x-3">
            ...
            <button type="submit" disabled={isLoading} className="flex items-center px-6 py-2 rounded-md text-sm font-bold text-white shadow-sm bg-amber-500 hover:bg-amber-600 disabled:opacity-50">
              <span className="mr-2">💾</span>{isLoading ? 'Procesando...' : (alumnoAEditar ? 'Actualizar Alumno' : 'Guardar Alumno')}
            </button>
          </div>
        </div>
      </form>
